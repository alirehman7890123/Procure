...
- `features/admin/services/user_service.py` and `features/admin/services/business_service.py` currently act as compatibility bridges over the existing shared service modules
...
- `userprofile/*.py`, `business/business.py`, and the business-name shell lookup in `utilities/mylogin.py` now import through the `features.admin.services` path
- `userprofile/baseprofile.py`, `business/basebusiness.py`, and the app shell wiring in `utilities/mylogin.py` now resolve admin/business UI through the `features.admin.ui` path
- `business/business.py` is now a compatibility shim that re-exports the feature-owned business-profile implementation
- `userprofile/userprofile.py` is now a compatibility shim that re-exports the feature-owned profile implementation
- `userprofile/changepassword.py` is now a compatibility shim that re-exports the feature-owned change-password implementation
- `userprofile/userslist.py` is now a compatibility shim that re-exports the feature-owned users-list implementation
- `userprofile/userdetail.py` is now a compatibility shim that re-exports the feature-owned user-detail implementation
- `business/discount_settings.py` is now a compatibility shim that re-exports the feature-owned discount-settings implementation
- `business/tax_settings.py` and `business/theme_settings.py` are now compatibility shims that re-export the feature-owned tax/theme implementations
- `userprofile/baseprofile.py` and `business/basebusiness.py` are now compatibility shims that re-export the feature-owned base admin widgets
- The next `admin` migration step is less about screen moves now and more about optional cleanup: reducing compatibility-layer usage over time or moving feature-local services from bridge mode to full ownership
- `features/finance/` has now been created as the second feature package scaffold
- `features/finance/services/` now exposes bridge modules for party transactions, expenses, daily sessions, and financial closing over the existing shared service layer
...
- `features/finance/ui/` now mostly imports through `features.finance.services` and feature-local UI modules instead of reaching back through compatibility shims, which makes the feature package more self-contained
- `transaction/basetransaction.py`, `expense/baseexpense.py`, and `financialclose/basefinancialclose.py` are now compatibility shims that re-export the feature-owned base finance widgets
- `transaction/showdetails.py`, `transaction/showcustomertransactions.py`, and `transaction/showsuppliertransactions.py` are now compatibility shims that re-export the feature-owned finance transaction screens
- `transaction/createcustomertransaction.py`, `transaction/createsuppliertransaction.py`, `transaction/customertransactionlist.py`, `transaction/suppliertransactionlist.py`, `transaction/customer_transaction_detail.py`, and `transaction/supplier_transaction_detail.py` are now compatibility shims that re-export the feature-owned finance transaction flow screens
- `expense/addexpense.py`, `expense/expenselist.py`, and `expense/expensedetail.py` are now compatibility shims that re-export the feature-owned finance expense screens
- `financialclose/mainpage.py`, `financialclose/listclosing.py`, and `financialclose/quartersummary.py` are now compatibility shims that re-export the feature-owned finance financial-close screens
- `dashboard/daily_session.py` is now a compatibility shim that re-exports the feature-owned finance daily-session screen
...
- `features/purchase/services/` now exposes bridge modules for purchase, PO, and GRN transaction/posting/draft services over the existing shared service layer
...
- `features/purchase/ui/` now mostly imports through `features.purchase.services` and feature-local UI modules instead of reaching back through purchase compatibility shims or the global purchase/grn service paths, which makes the feature package more self-contained
- `purchase/basepurchase.py`, `purchase/base_po.py`, and `purchase/base_grn.py` are now compatibility shims that re-export the feature-owned purchase base widgets
- `purchase/addpurchase.py`, `purchase/purchaselist.py`, and `purchase/purchasedetail.py` are now compatibility shims that re-export the feature-owned purchase invoice screens
- `purchase/add_po.py`, `purchase/po_list.py`, and `purchase/po_detail.py` are now compatibility shims that re-export the feature-owned purchase-order screens
- `purchase/create_grn.py`, `purchase/grn_list.py`, and `purchase/grn_detail.py` are now compatibility shims that re-export the feature-owned GRN screens
...
- `features/inventory/services/` now exposes bridge modules for product write, product catalog, product admin, product media, and stock-adjustment services over the existing shared service layer
...
- `features/inventory/ui/` now mostly imports through `features.inventory.services` and feature-local UI modules for the product-owned stack, with only the shared prescription-schema helper still coming from outside the feature
- `product/baseproduct.py` is now a compatibility shim that re-exports the feature-owned inventory base widget
- `product/addproduct.py`, `product/productlist.py`, and `product/productdetail.py` are now compatibility shims that re-export the feature-owned inventory product screens
...
- `features/sales/services/` now exposes bridge modules for sales posting, sales items, sales detail, sales transactions, sales defaults, product media, and accounting settings over the existing shared service layer
...
- `features/sales/ui/receipt_list.py` now delegates its invoice list/search/barcode reads to `services/sales_detail_service.py`, both invoice export read paths in `features/sales/ui/create_sales.py` now flow through that service, the sales customer/manufacturer helper flows have been moved there too, the held-sale list/detail/item reads now also flow through that service, the hold-sale persistence path now runs through `features.sales.services.sales_transaction_service`, the salesman lookup / credit-position read / customer-transaction persistence plus sales header/prescription persistence are now service-backed, and the sales item posting/FIFO allocation orchestration now also runs through `features.sales.services.sales_transaction_service`; the main remaining sales consolidation gap is the residual helper/query logic still living around `features/sales/ui/create_sales.py`
- `features/sales/ui/` now mostly imports through `features.sales.services` and feature-local UI modules, and the older sales detail screen has been cleaned of stale pre-extraction imports so the package boundary is more consistent internally too
- `sales/basesales.py`, `sales/createsales.py`, `sales/receiptlist.py`, and `sales/salesdetail.py` are now compatibility shims that re-export the feature-owned sales screens
