...
from features.purchase.services.purchase_order_service import fetch_purchase_order_list_rows
...
class POListWidget(QWidget):
    ...
    def load_po_list(self):
        ...
        try:
            rows = fetch_purchase_order_list_rows(
                date_from=date_from,
                date_to=date_to,
        ...
        for row_data in rows:
            ...
            po_id = row_data["po_id"]
            po_number = row_data["po_number"]
            supplier = row_data["supplier_name"]
            po_date = row_data["po_date"]
            expected_delivery = row_data["expected_delivery_date"]
            status = row_data["status"]
            ordered_qty = float(row_data["ordered_qty"] or 0)
            received_qty = float(row_data["received_qty"] or 0)
            ...
            remaining_qty = float(row_data["remaining_qty"] or 0.0)
