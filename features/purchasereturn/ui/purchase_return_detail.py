...
class PurchaseReturnDetailWidget(QWidget):
    ...
    def load_purchase_data(self, purchase_return_id: int) -> None:
        ...
        header = fetch_purchase_return_detail(purchase_return_id)
        if header is None:
            self.supplier.setText("Purchase not found.")
            return

        # Set header fields
        self.orderid.setText(str(header["id"]))
        self.dateandtime.setText(header["creation_date"])
        self.subtotal.setText(f'{header["subtotal"]:.2f}')
        self.discount.setText(f'{header["received"]:.2f}')
        self.tax.setText(f'{header["remaining"]:.2f}')
        self.roundoff.setText(f'{header["writeoff"]:.2f}')
        self.finalamount.setText(f'{header["total"]:.2f}')
        self.supplier.setText(header["supplier_name"])
        self.sellerorder.setText(str(header["rep_name"]))
    ...
    def load_items_into_table(self, purchase_return_id: int) -> None:
        ...
        rows = fetch_purchase_return_item_rows(purchase_return_id)
