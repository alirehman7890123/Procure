...
from medic.services.return_read_service import (
    ...
    fetch_purchase_return_rep_options,
)
...
class AddPurchaseReturnWidget(QWidget):
    ...
    def populate_suppliers(self):
        ...
        try:
            for row in fetch_active_purchase_return_supplier_options():
                self.supplier_edit.addItem(row["name"], row["id"])
    ...
    def populate_reps(self):
        ...
        self.rep_edit.clear()
        try:
            for row in fetch_purchase_return_rep_options(supplier):
                self.rep_edit.addItem(row["name"], row["id"])
        except Exception as exc:
            AppMessageBox.information(None, 'Error', str(exc))
