...
from medic.services.return_read_service import (
    ...
    fetch_sales_return_customer_label,
    fetch_sales_return_product_display,
    fetch_sales_return_sales_header,
    fetch_sales_return_salesitem_product_id,
    fetch_sales_return_salesman_label,
    fetch_sales_return_source_item_rows,
)
...
class AddSalesReturnWidget(QWidget):
    ...
    def _lookup_salesitem_product_id(self, salesitem_id):
        return fetch_sales_return_salesitem_product_id(salesitem_id)

    def _lookup_sales_header(self, sales_id):
        return fetch_sales_return_sales_header(sales_id)

    def _lookup_customer_label(self, customer_id):
        return fetch_sales_return_customer_label(customer_id)

    def _lookup_salesman_label(self, salesman_id):
        return fetch_sales_return_salesman_label(salesman_id)

    def _lookup_product_display(self, product_id):
        return fetch_sales_return_product_display(product_id)
    ...
    def load_items_into_table(self, id):
        ...
        rows = fetch_sales_return_source_item_rows(id)
        self.table.setRowCount(0)

        print("Loading items into table")

        for row, source_row in enumerate(rows):
            self.table.insertRow(row)

            item_id_value = str(source_row["salesitem_id"])
            product = source_row["product_id"]
            quantity_value = int(source_row["qty_sold"])
            print("Loading Sold Quantity that is: ", quantity_value)
            effective_total = float(source_row["effective_line_total"] or 0.0)

            rate_value = effective_total / quantity_value if quantity_value else 0

            print("Effective rate is : ", rate_value)

            returned = QLineEdit()
            returned.setText("0")
            returned.textChanged.connect(lambda _, widget=returned: self.update_amount(widget))

            total_value = "0.0"
            name_value, maker_value = self._lookup_product_display(product)

            item_id = QTableWidgetItem(item_id_value)
            name = QTableWidgetItem(name_value)
            maker = QTableWidgetItem(maker_value)
            quantity = QTableWidgetItem(str(quantity_value))
            rate = QTableWidgetItem(str(rate_value))
            total = QTableWidgetItem(total_value)
    ...
    def populate_salesman(self):
        ...
        self.salesman.clear()
        try:
            for row in fetch_active_sales_return_salesman_options():
                self.salesman.addItem(row["name"], row["id"])
