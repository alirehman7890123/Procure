...
from medic.features.finance.services.party_transaction_service import fetch_supplier_balance_rows, reconcile_supplier_internal_balance
...
class SupplierTransactionWidget(QWidget):
    ...
    def load_suppliers_transactions(self):
        try:
            rows = fetch_supplier_balance_rows()
        ...
        for row_data in rows:
            ...
            supplier_id = int(row_data["party_id"] or 0)
            ...
            pay.clicked.connect(partial(self.transaction_page_signal.emit, supplier_id))
            ...
            if can_reconcile:
                reconcile.clicked.connect(partial(self.reconcile_supplier_balance, supplier_id, name_text))
    ...
    def reconcile_supplier_balance(self, supplier_id, supplier_name):
        try:
            balance_rows = fetch_supplier_balance_rows()
        except Exception as exc:
            AppMessageBox.critical(self, "Error", str(exc))
            return
        row = next((item for item in balance_rows if int(item["party_id"] or 0) == int(supplier_id)), None)
        if row is None:
        ...
        _, accepted = AppMessageBox.confirm(
            ...
            (
                f"{supplier_name} has payable {float(row['payable'] or 0.0):.2f} and receivable {float(row['receiveable'] or 0.0):.2f}.\n"
                f"Reconcile {min(float(row['payable'] or 0.0), float(row['receiveable'] or 0.0)):.2f} internally?"
            ),
        ...
        if not accepted:
            return
        try:
            result = reconcile_supplier_internal_balance(supplier_id)
            AppMessageBox.success(self, "Reconciled", f"{result['reconcile_amount']:.2f} reconciled for {supplier_name}.")
            self.load_suppliers_transactions()
