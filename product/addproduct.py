...
class AddProductWidget(QWidget):
    ...
    def populate_stock_and_batch_fields(self):
        ...
        # Labels + Fields
        labels = ["Pack Size", "Cost Price", "Packs", "Units", "Reorder Level", "Sale Price per Pack"]
        ...
        fields = [
            self.pack_size_input, self.total_cost_input, self.packs_input, self.units_input,
    ...
    def save_product( self, product, code, category, brand, formula, form, strength,
        ...
        if not product_query.exec() or not product_query.next():
            db.rollback()
    ...
    # def save_product(self, product, code, category, brand, formula, form, strength, packsize, pack, unit, reorder, cost, sale, batch, expiry):
    ...
    #     packs = pack.text()
    #     units = unit.text()
    #     units = int(packs) * int(packsize) + int(units)
    #     units = str(units)
    #     reorder = reorder.text()
    ...
    #     batch = batch.text()
    #     expiry = expiry.date()
    #     print("DATE is: ", expiry)
        
    #     expiry = expiry.toPython()
    ...
    #     query.prepare(" INSERT INTO product (name, code, category, brand, formula, form, strength) VALUES (?,?,?,?,?,?, ?) RETURNING id")
    ...
    #         print("Failed to insert into product:", query.lastError().text())
    #         QMessageBox.information(None, "Failed", query.lastError().text())
    ...
    #         stock_query.prepare(" INSERT INTO stock (product, packsize, units, reorder, saleprice) VALUES (?,?,?,?,?)")
    ...
    #             print("Failed to insert into Stock:", stock_query.lastError().text())
    #             QMessageBox.information(None, "Failed", stock_query.lastError().text())
    #             db.rollback()
                
    #         else:
                
    #             print("All inserts into Stock successful!")
                
    #             print("Adding Expiry Date as: ", type(expiry))
                
    #             if batch != '':
                    
    #                 batch_query = QSqlQuery()
                
    #                 batch_query.prepare("INSERT INTO batch (product, batch, expiry) VALUES (?, ?, ?)")
    #                 batch_query.addBindValue(product_id)
    #                 batch_query.addBindValue(batch)
    #                 batch_query.addBindValue(expiry)
    ...
    #                     print("Failed to insert into batch:", batch_query.lastError().text())
    #                     QMessageBox.information(None, "Failed", batch_query.lastError().text())
    #                     db.rollback()
            
    #                 else:
                        
    #                     print("All inserts into Batch successful!")
                
                
    #         stockcost_query = QSqlQuery()
                
    #         stockcost_query.prepare("INSERT INTO stockcost (product, qty, totalcost, stocktype) VALUES (?, ?, ?, ?)")
    #         stockcost_query.addBindValue(product_id)
    #         stockcost_query.addBindValue(units)
    #         stockcost_query.addBindValue(cost)
    #         stockcost_query.addBindValue('onhand')
    ...
    #             print("Failed to insert into StockCost:", stockcost_query.lastError().text())
    #             QMessageBox.information(None, "Failed", stockcost_query.lastError().text())
    #             db.rollback()
    
    #         else:
                
    #             print("All inserts are successful!")
            
    #             db.commit()
    #             QMessageBox.information(None, "Success", 'All Inserts were successfull')
