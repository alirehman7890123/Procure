...
- `features/customer/` has now been created as the next feature package scaffold for the remaining people/entity cleanup wave
- `features/customer/ui/base_customer.py`, `features/customer/ui/add_customer.py`, `features/customer/ui/customer_list.py`, and `features/customer/ui/customer_detail.py` now own the main customer base/add/list/detail flow
- `features/customer/ui` now consumes shared customer logic directly through `medic.services.customer_service`, and `utilities/mylogin.py` now imports the customer entry widget through the `features.customer.ui` path
- `features/customer/ui` is now the sole internal owner for the customer base/add/list/detail flow, and the old top-level `customer/*` screen shims have now been removed
- `features/supplier/` has now been created as the next feature package scaffold in the same people/entity cleanup wave
- `features/supplier/ui/base_supplier.py`, `features/supplier/ui/add_supplier.py`, `features/supplier/ui/supplier_list.py`, and `features/supplier/ui/supplier_detail.py` now own the main supplier base/add/list/detail flow
- `features/supplier/ui` now consumes shared supplier logic directly through `medic.services.supplier_service`, and `utilities/mylogin.py` now imports the supplier entry widget through the `features.supplier.ui` path
- `features/supplier/ui` is now the sole internal owner for the supplier base/add/list/detail flow, and the old top-level `supplier/*` screen shims have now been removed
- `features/employee/` has now been created as the next feature package scaffold in the same people/entity cleanup wave
- `features/employee/ui/base_employee.py`, `features/employee/ui/add_employee.py`, `features/employee/ui/employee_list.py`, and `features/employee/ui/employee_detail.py` now own the main employee base/add/list/detail flow
- `features/employee/ui` now consumes shared employee logic directly through `medic.services.employee_service`, and `utilities/mylogin.py` now imports the employee entry widget through the `features.employee.ui` path
- `features/employee/ui` is now the sole internal owner for the employee base/add/list/detail flow, and the old top-level `employee/*` screen shims have now been removed
- `features/salesrep/` has now been created as the next feature package scaffold in the same people/entity cleanup wave
- `features/salesrep/ui/base_salesrep.py`, `features/salesrep/ui/add_salesrep.py`, `features/salesrep/ui/salesrep_list.py`, and `features/salesrep/ui/salesrep_detail.py` now own the main sales-rep base/add/list/detail flow
- `features/salesrep/ui` now consumes shared sales-rep logic directly through `medic.services.salesrep_service`, and `utilities/mylogin.py` now imports the sales-rep entry widget through the `features.salesrep.ui` path
- `features/salesrep/ui` is now the sole internal owner for the sales-rep base/add/list/detail flow, and the old top-level `salesrep/*` screen shims have now been removed
- `services/salesrep_service.py` now owns supplier-option reads, sales-rep validation, create flow, list reads, detail reads, and update flow, so the sales-rep UI no longer owns direct widget-level SQL
- `features/purchasereturn/` has now been created as the next feature package scaffold for the returns wave
- `features/purchasereturn/ui/base_purchase_return.py`, `features/purchasereturn/ui/create_purchase_return.py`, `features/purchasereturn/ui/purchase_return_list.py`, and `features/purchasereturn/ui/purchase_return_detail.py` now own the purchase-return base/add/list/detail flow
- `features/purchasereturn/ui` already leans on shared return logic through `medic.services.purchase_return_service`, `medic.services.purchase_return_transaction_service`, and now `medic.services.return_read_service`, so the purchase-return list/detail reads and supplier/rep option loads are no longer owned directly by the widgets; `utilities/mylogin.py` now imports the purchase-return entry widget through the `features.purchasereturn.ui` path
- `features/purchasereturn/ui` is now the sole internal owner for the purchase-return base/add/list/detail flow, and the old top-level `purchasereturn/*` screen shims have now been removed
- `features/salesreturn/` has now been created as the next feature package scaffold in the same returns wave
- `features/salesreturn/ui/base_sales_return.py`, `features/salesreturn/ui/create_sales_return.py`, `features/salesreturn/ui/sales_return_list.py`, and `features/salesreturn/ui/sales_return_detail.py` now own the sales-return base/add/list/detail flow
- `features/salesreturn/ui` already leans on shared return logic through `medic.services.sales_return_service`, `medic.services.sales_return_transaction_service`, and now `medic.services.return_read_service`, so the sales-return list/detail reads and salesman option loads are no longer owned directly by the widgets; `utilities/mylogin.py` now imports the sales-return entry widget through the `features.salesreturn.ui` path
- `features/salesreturn/ui` is now the sole internal owner for the sales-return base/add/list/detail flow, and the old top-level `salesreturn/*` screen shims have now been removed
- `features/purchasereturn/ui/create_purchase_return.py` is now largely orchestration-focused: supplier/rep option loads and list/detail reads all flow through shared services, and the remaining logic is mostly widget coordination plus the already-shared posting path
- `features/salesreturn/ui/create_sales_return.py` now also routes its source-sale header lookup, salesitem/product lookups, customer/salesman labels, and source sales-item row loads through `medic.services.return_read_service`, so the return-create widget is now much closer to the same orchestration-focused shape as purchase return
- `features/payroll/` has now been created as the next feature package scaffold for the remaining operational modules
- `features/payroll/ui/base_payroll.py`, `features/payroll/ui/add_payroll.py`, `features/payroll/ui/payroll_list.py`, `features/payroll/ui/payroll_detail.py`, `features/payroll/ui/attendance.py`, `features/payroll/ui/attendance_list.py`, `features/payroll/ui/salary_advance.py`, and `features/payroll/ui/advance_list.py` now own the payroll base/add/list/detail plus attendance and salary-advance flows
- `features/payroll/ui` already leans on shared payroll logic through `medic.services.payroll_service` and `medic.services.payroll_posting_service`, and `utilities/mylogin.py` now imports the payroll entry widget through the `features.payroll.ui` path
- `features/payroll/ui` is now the sole internal owner for the payroll base/add/list/detail, attendance, and salary-advance flows, and the old top-level `payroll/*` screen shims have now been removed
- `medic.services.payroll_service.resolve_payroll_auth_user_id(...)` now owns the auth-user lookup used by payroll save flows, so `features/payroll/ui/add_payroll.py`, `features/payroll/ui/attendance.py`, and `features/payroll/ui/salary_advance.py` no longer run direct auth queries inside the widgets
- `reports/__init__.py` has now been added as a lightweight package boundary, `reports/basereports.py` now lazy-loads `MainReportsPage` instead of importing it at module load time, and low-signal report-page debug prints have been trimmed from `reports/mainpage.py`, so the reports package is less eager and less transitional than before
- `features/salehold/` has now been created as a feature-owned home for the legacy on-hold-sales screens, with `features/salehold/ui/base_hold.py`, `features/salehold/ui/hold_list.py`, and `features/salehold/ui/hold_detail.py` owning that flow
- the sale-hold list/detail screens now lean on `medic.services.sales_detail_service` for held-sale list/detail/item reads instead of running widget-owned SQL, the dead eager shell import of `BaseHoldSalesWidget` has been removed from `utilities/mylogin.py` because the module is not currently active in navigation, and the old top-level `salehold/*` screen shims have now been removed too
- the main remaining returns consolidation gap is no longer the basic read/query boundary; future cleanup is more likely to be UI simplification, consistency passes, or eventual shim retirement rather than another large read-service extraction
- repo-internal test and shell imports that were still pointing through shim modules have now been redirected to feature-owned module paths, and the remaining people/entity/returns/payroll/salehold shim files have now been deleted rather than left as active compatibility entry points
...
- a repo-wide import normalization pass is now in place across feature UI, shared services, reports, dashboard, payroll, and tests so the app mostly uses canonical `medic...` imports internally
- the startup shell boundary is now much thinner too: `utilities/mylogin.py` no longer imports the full page/widget set at module load time and instead lazy-resolves page classes through a shared import helper, while `starting.py` now uses the same helper pattern for its utility entry imports instead of scattered fallback blocks
- the main intentional compatibility exception left is that `starting.py` and `utilities/mylogin.py` still keep that helper-based fallback path for frozen/package resilience; the next cleanup decision is whether to retire that compatibility path once packaging is stable enough
