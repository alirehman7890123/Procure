...
from medic.services.purchase_transaction_service import fetch_purchase_list_rows
...
class PurchaseListWidget(QWidget):
    ...
    def load_purchases_into_table(self):
        ...
        for row_data in rows:
            ...
            purchase_id = int(row_data["purchase_id"])
            supplier = str(row_data["supplier_name"] or "")
            invoice = str(row_data["sellerinvoice"] or "")
            rep = str(row_data["rep_name"] or "")
            creation = str(row_data["creation_date"] or "")
