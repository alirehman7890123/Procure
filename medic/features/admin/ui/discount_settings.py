...
class DiscountSettingsWidget(QWidget):
    ...
    def populate_global_discount_combo(self, selected_id=None):
        ...
        try:
            groups = fetch_active_sales_groups("discount")
    ...
    def load_discount_groups(self):
        ...
        self.load_global_sales_discount()
        try:
            groups = fetch_sales_groups("discount")
        except Exception as exc:
            AppMessageBox.error(self, "Load Failed", str(exc))
            return
    ...
    def load_selected_row(self):
        ...
        try:
            group = fetch_sales_group_detail("discount", group_id)
        except Exception as exc:
            AppMessageBox.error(self, "Load Failed", str(exc))
            return
        if not group:
    ...
    def save_discount_group(self):
        ...
        try:
            save_sales_group(
                ...
                status=status,
                group_id=self.current_discount_group_id,
            )
        except ValueError as exc:
            AppMessageBox.warning(self, "Validation Error", str(exc))
            return
